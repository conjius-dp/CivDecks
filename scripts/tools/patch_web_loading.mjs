...
#status-notice {
  display: none;
  color: #e8c055 !important; background: rgba(0,0,0,0.9) !important;
  font-size: 14px !important; max-width: 80vw !important;
  position: fixed !important; bottom: 10% !important; left: 50% !important;
  transform: translateX(-50%) !important; z-index: 100 !important;
  padding: 1em !important; border-radius: 8px !important;
...
(function() {
	...
	// Detect game ready -> snap bar to 100% and fade out overlay.
	// Godot calls statusOverlay.remove() when the engine starts,
	// so we detect that the #status element is gone from the DOM.
...
// Inject progress bar OUTSIDE #status so Godot's statusOverlay.remove() doesn't destroy it
html = html.replace(
	'</body>',
	'<div class="progress-fill"><div class="progress-fill-inner"></div></div></body>'
);
